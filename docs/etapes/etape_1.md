---
layout: default
title: Préparation des Matériaux
parent: Etapes de fabrication
nav_order: 1
---

# Préparation des Matériaux

Cette section décrit la première étape du processus de fabrication : la préparation des matériaux.

## Liste des Matériaux

### Éléments structurels (Impression 3D)
* **Composants du corps** : Tête, corps, jambes et pieds (vérifiez l'absence de défauts d'impression).
* **Accessoires spécifiques** : Éléments personnalisés (comme les fixations d'outils ou fentes d'accessoires sur le dos).

### Outillage nécessaire
* **Jeu de tournevis de précision** (cruciforme principalement).
* **Pince coupante et pince plate** (pour ajuster les câbles et nettoyer les pièces).
* **Cutter ou Ebavureur** pour nettoyer les contours.

## Procédure de Préparation

1.  **Nettoyage** : Inspectez toutes les pièces imprimées en 3D. Retirez délicatement les supports d'impression et utilisez la lime ou le cutter pour éliminer les imperfections (bavures, fils de plastique). Une attention particulière doit être portée aux **logements des servomoteurs** et aux **fentes d'assemblage**.
2.  **Ajustement à blanc** : Testez l'imbrication des pièces principales (jambes dans les pieds, corps avec la tête) sans forcer. Si le passage est trop serré, limez légèrement la zone de contact jusqu'à obtenir un ajustement précis mais fluide.
3.  **Vérification des passages de vis et câbles** : Assurez-vous que les trous destinés à la visserie et les ouvertures pour le passage des câbles (capteurs, alimentation) sont parfaitement dégagés de tout résidu de plastique.

## Conseils de Sécurité

* **Protection des yeux** : Portez des lunettes de sécurité lors de la découpe des supports d'impression 3D ou de l'utilisation de la pince coupante (les éclats de plastique peuvent être projetés).
* **Utilisation des outils tranchants** : Maniez toujours le cutter ou l'ébavureur en orientant la lame vers l'extérieur (à l'opposé de vos mains et de votre corps).
* **Entretien des pièces** : Ne forcez jamais sur une pièce en plastique pour éviter de la fendre ou de vous blesser si l'outil dérape.
---

Une fois cette étape terminée, passez à l'[Assemblage](/assemblage).
